...
namespace JeffersonGoncalves\Benefit\Tests;
...
abstract class TestCase extends Orchestra
{
    ...
    protected function setUp(): void
    {
        ...
        Factory::guessFactoryNamesUsing(
            fn (string $modelName) => 'JeffersonGoncalves\\Benefit\\Database\\Factories\\'.class_basename($modelName).'Factory'
        );
    }
    ...
    protected function getEnvironmentSetUp($app): void
    {
        ...
        $configPath = __DIR__.'/../config/benefit.php';
        if (file_exists($configPath)) {
            $app['config']->set('benefit', require $configPath);
        }
    }

    /**
     * Defaults to an in-memory SQLite connection for local development; CI
     * (tests.yml) sets BENEFIT_TEST_DB_* to run the same suite against real
    ...
    {
        $driver = env('BENEFIT_TEST_DB_DRIVER', 'sqlite');
        ...
        return [
            'driver' => $driver,
            'host' => env('BENEFIT_TEST_DB_HOST', '127.0.0.1'),
            'port' => env('BENEFIT_TEST_DB_PORT'),
            'database' => env('BENEFIT_TEST_DB_DATABASE', 'testing'),
            'username' => env('BENEFIT_TEST_DB_USERNAME', 'root'),
            'password' => env('BENEFIT_TEST_DB_PASSWORD', ''),
    ...
    protected function defineDatabaseMigrations(): void
    {
        ...
        $tempPath = sys_get_temp_dir().'/laravel-benefit-migrations';
